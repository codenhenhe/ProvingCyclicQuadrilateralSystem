...
class GeometryPlotter:
    ...
    def draw(self, should_show=True, ordered_polygon=None, extra_segments=None):
        ...
        # =====================================================================
        # [NEW] GOM NHÓM ĐIỂM TRÙNG NHAU (Coincident Points Grouping)
        # =====================================================================
        grouped_points = {}
        threshold = 0.15 # Khoảng cách coi là trùng
        
        # Duyệt qua tất cả các điểm để gom nhóm
        for name, (x, y) in self.points.items():
            if "Ext_" in name: # Điểm ảo vẽ riêng
            ...
            for (gx, gy), names in grouped_points.items():
                if math.sqrt((x-gx)**2 + (y-gy)**2) < threshold:
                    ...
                    break
            if not found:
                grouped_points[(x, y)] = [name]

        # 1. Vẽ điểm và Nhãn (Sử dụng danh sách đã gộp)
        ...
        for p_vertex, p1, p2, val, color in self.angle_markers:
            if {p_vertex, p1, p2}.issubset(self.points.keys()):
                ...
                arc = patches.Arc((xv, yv), 1.5, 1.5, theta1=0, theta2=0, angle=0, linewidth=1.5, color=color)
                arc.theta1 = theta1; arc.theta2 = theta2
                self.ax.add_patch(arc)
                
                if val:
                    mid = math.radians(theta1 + (theta2 - theta1)%360 / 2)
                    # Giữ nguyên offset 1.3 như bạn muốn
                    tx = xv + 1.3 * math.cos(mid) 
                    ty = yv + 1.3 * math.sin(mid)
